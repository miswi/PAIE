...
class BartSingleArg(BartPretrainedModel):
    ...
    def forward(
        ...
        for i, (decoder_prompt_ids, decoder_prompt_mask, decoder_prompt_start_positions, decoder_prompt_end_positions) in \
            ...
            prompt_decoder_outputs = prompt_decoder_outputs.last_hidden_state  # last_hidden_state:(4,20,768) (batch_size, max_dec_seq_length/Query_L, hidden_dim)
            ...
            for j, (p_start, p_end, prompt_decoder_output) in enumerate(zip(decoder_prompt_start_positions, decoder_prompt_end_positions, prompt_decoder_outputs)):
                prompt_query_sub = prompt_decoder_output[p_start:p_end]  # 根据 prompt 的起始/结束位置，在模型输出中索引出 prompt embeddings
            ...
            start_prompt_query = (prompt_query*self.w_prompt_start[None, :]).unsqueeze(-1)  # (batch_size, hidden_dim, 1)
            end_prompt_query = (prompt_query*self.w_prompt_end[None, :]).unsqueeze(-1)  # (batch_size, hidden_dim, 1)
            # 将 context_decoder_outputs 与 prompt 进行融合，实现输入与提示之间的融合
            start_logits = torch.bmm(context_decoder_outputs[i:i+1].repeat(len(start_prompt_query),1,1), start_prompt_query).squeeze(-1)  # (batch_size:4, max_enc_seq_length:500)             # [Arg_num, L]
            ...
            for j, arg_role in enumerate(arg_list[i]):
                ...
                output[arg_role] = [start_logits_list, end_logits_list]  # 论元角色对应的预测概率
