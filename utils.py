...
def hungarian_matcher(predicted_spans, target_spans):
    ...
    """
    torch.cdist() 计算两个行向量集合的每对之间的 p-norm 距离。
    该函数返回一个张量，其中每个元素是x1中的一个样本与x2中的一个样本之间的距离，距离的形状为(x1的样本数, x2的样本数)。
    p：计算每个向量对之间的 p-norm 距离的 p 值，欧几里德距离：p = 2
    
    linear_sum_assignment(cost_matrix, maximize=False):
        用于解决最小加权匹配问题的函数，它可以用于将两个数据集之间的样本进行匹配，使得匹配的总权重最小。
        其中，cost_matrix是一个二维矩阵，表示每个样本对之间的匹配成本（或者说权重），maximize表示是否求最大权重匹配（默认为False，即求最小权重匹配）。
        该函数返回一个元组，包含两个数组（或列表），分别表示x和y中每个样本的索引，这些样本之间的匹配成本最小。
   """
...
def get_best_span(start_logit, end_logit, old_tok_to_new_tok_index, max_span_length):
    ...
    """
    在 span 长度不大于10的范围内，对 span start/end 进行两两组合并结合分词映射矩阵，找到 span 的 start/end 索引，
    并根据索引得到对应的 start/end prompt repre with context repre 联合概率。不断比较不同 span 组合得到的概率值，
    找到概率值最大的 span start/end 索引，作为最终的 best span 索引返回。
    """
    for start in range(context_length):
        for end in range(start+1, min(context_length, start+max_span_length+1)):
            ...
            score = start_logit[start_index] + end_logit[end_index]     # 当前 start/end 组合出来的概率值
            answer_span = (start_index, end_index)
            if score > best_score:      # 如果当前 start/end 组合出来的概率值大于上一个组合的概率值，则更新 span 索引
                best_score = score
