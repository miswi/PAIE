...
class BaseEvaluator:
    ...
    def evaluate_one_batch(self, batch):
        outputs_list, named_v = self.calculate_one_batch(batch)  # outputs_list: 论元角色对应的预测概率，
...
class Evaluator(BaseEvaluator):
    ...
    def build_and_clean_record(self):
        ...
        保存每批数据的预测概率，
        role_list[i]：句子中论元角色
        full_start_logit_list[i]：句子中论元角色 span_start 的预测概率
        full_start_logit_list[i]：句子中论元角色 span_end 的预测概率
        """
        self.record = {
    ...
    def collect_fn(self, outputs_list, named_v, batch):   
        bs = len(batch[0])
        for i in range(bs):
            predictions = outputs_list[i]  # 第i个句子的预测论元概率
            feature_id = named_v["feature_ids"][i].item()
            for arg_role in named_v["arg_roles"][i]:  # 获取第i个句子的论元角色的text
                [start_logits_list, end_logits_list] = predictions[arg_role]  # 根据论元text索引出 预测概率， # NOTE base model should also has these kind of output
